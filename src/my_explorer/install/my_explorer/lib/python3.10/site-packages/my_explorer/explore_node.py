...
class RipExplorer(Node):
    def __init__(self):
        ...
        self.map_data = None
        self.blacklist = []
        self.last_pos = (-0.3, 0.0)
        
        # --- AGGRESSIVE TUNING ---
        self.SCAN_RATE = 30.0      # "Rip" a new goal every 0.8 seconds
        self.MIN_FRONTIER_DIST = 0.1  # How far away the next target must be
        #self.WALL_SAFETY_PX = 1        # Keep walls thin (1 pixel buffer)
        # -------------------------
        
        self.timer = self.create_timer(self.SCAN_RATE, self.exploration_loop)
        self.get_logger().info(" Streaming goals to Nav2 without stopping.")
    ...
    def get_robot_pose(self):
        try:
            ...
            t = self.tf_buffer.lookup_transform('map', 'base_link', rclpy.time.Time(), timeout=Duration(seconds=10))
